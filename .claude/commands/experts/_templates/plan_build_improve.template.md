...
description: Complete {EXPERT} workflow - plan, build, and self-improve
argument-hint: [request] [human_in_the_loop (bool)]
---

# Purpose

Orchestrates complete {EXPERT} implementation cycle: plan -> build -> self-improve.
...
Task(
  ...
  prompt: "Run SlashCommand('/experts:{EXPERT}:plan [USER_PROMPT]'). Return the path to the generated plan file."
)
```
...
Task(
  subagent_type: "general-purpose",
  prompt: "Run SlashCommand('/experts:{EXPERT}:self-improve true'). Return the self-improvement report."
)
```
...
### Final Status
✅ {EXPERT} implementation workflow complete
