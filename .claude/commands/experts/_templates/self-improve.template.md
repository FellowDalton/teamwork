...
description: Self-improve {EXPERT} expertise by validating against codebase
argument-hint: [check_git_diff (bool)] [focus_area (optional)]
---

# Purpose

Maintain {EXPERT} expertise accuracy by comparing against actual implementation.
...
EXPERTISE_DIR: .claude/commands/experts/{EXPERT}
JOURNAL_FILE: .claude/commands/experts/{EXPERT}/expertise.learnings.jsonl
MAX_LINES: 1000

## Instructions

- This is a self-improvement workflow to keep {EXPERT} expertise synchronized with the actual codebase
- Think of the expertise file as your **mental model** and memory reference for all {EXPERT}-related functionality
...
```json
{"ts":"ISO-8601-timestamp","expert":"{EXPERT}","track":"<track-id>","story":"<story-slug>","type":"update|add|remove","section":"<yaml-section-name>","description":"<what changed and why>","content":"<the actual YAML snippet or value to add/change/remove>"}
```

- `ts`: current UTC timestamp in ISO-8601 format
- `expert`: the expert name (`{EXPERT}`)
