...
description: Complete teamwork workflow - plan, build, and self-improve
argument-hint: [request] [human_in_the_loop (bool)]
---

# Purpose

Orchestrates complete teamwork implementation cycle: plan -> build -> self-improve.
...
Task(
  ...
  prompt: "Run SlashCommand('/experts:teamwork:plan [USER_PROMPT]'). Return the path to the generated plan file."
)
```
...
Task(
  subagent_type: "general-purpose",
  prompt: "Run SlashCommand('/experts:teamwork:self-improve true'). Return the self-improvement report."
)
```
...
### Final Status
✅ teamwork implementation workflow complete
