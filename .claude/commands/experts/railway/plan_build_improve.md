...
description: Complete railway workflow - plan, build, and self-improve
argument-hint: [request] [human_in_the_loop (bool)]
---

# Purpose

Orchestrates complete railway implementation cycle: plan -> build -> self-improve.
...
Task(
  ...
  prompt: "Run SlashCommand('/experts:railway:plan [USER_PROMPT]'). Return the path to the generated plan file."
)
```
...
Task(
  subagent_type: "general-purpose",
  prompt: "Run SlashCommand('/experts:railway:self-improve true'). Return the self-improvement report."
)
```
...
### Final Status
railway implementation workflow complete
