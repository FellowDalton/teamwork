...
export const AnalogButton: React.FC<AnalogButtonProps> = ({
  ...
  const getColors = () => {
    ...
    // --- ACCENT BUTTONS (Default Colors) ---
    if (variant === "accent") {
      // Use the same colors as dark variant for accent buttons
      if (theme === "light") {
        return isDown
          ? `bg-zinc-300 text-zinc-900 ${baseTransition}`
          : `bg-zinc-200 text-zinc-800 hover:bg-zinc-100 ${baseTransition}`;
      }
      // Dark Theme Accent (same as dark variant)
      return isDown
    ...
    if (!isActive)
      return `${base} ${
        theme === "light"
          ...
          : "bg-zinc-950 shadow-inner opacity-20"
      }`;
  ...
  const getSubLabelColor = () => {
    if (variant === "accent") {
      // Use same colors as dark variant for accent buttons
      if (theme === "light")
        return isDown
          ? "text-zinc-500"
  ...
  const textureClass =
    theme === "light" ? "bg-texture-btn-light" : "bg-texture-btn-dark";

  // Layout logic
  const spacingClass = flush ? "w-full m-0" : "w-[calc(100%-4px)] m-[2px]";
  const radiusClass = flush
    ? "rounded-[2px]"
    : minimal
    ? "rounded-md"
    : "rounded-lg";
    ...
    <button
      ...
      style={{
        ...
        transition: flush
          ...
          : "transform 0.08s ease-out, box-shadow 0.08s ease-out, background-color 0.2s",
      }}
    >
      {/* Texture Overlay - Blended opacity with boost for active state */}
      {!noTexture && (
        <div
          className={`absolute inset-0 ${textureClass} ${
            isActive ? "opacity-[0.05] brightness-200" : "opacity-[0.15]"
          } pointer-events-none`}
        />
      )}
      ...
      <div
        className={`flex items-center gap-3 relative z-20 ${
          minimal ? "justify-center w-full" : "w-full"
        }`}
      >
        {icon && (
          <span
            className={`transition-opacity duration-200 ${
              variant === "accent"
                ? "opacity-70 group-hover:opacity-100" // Use same opacity as other variants
        ...
        {!minimal && (
          <span
            className={`font-semibold text-sm tracking-tight truncate select-none ${
              variant === "accent" ? "" : "" // Remove forced white text for accent variant
            }`}
          >
            {label}
          </span>
      ...
      {!minimal && (
        <div className="flex items-end justify-between w-full mt-2 relative z-20">
          <span
            className={`text-[10px] uppercase font-bold tracking-wider select-none transition-colors duration-200 ${getSubLabelColor()}`}
          >
            {subLabel}
          </span>
          ...
          <div
            className={`${
              theme === "light" ? "bg-zinc-200" : "bg-black/40"
            } p-1 rounded-full shadow-[inset_0_1px_2px_rgba(0,0,0,0.5)]`}
          >
